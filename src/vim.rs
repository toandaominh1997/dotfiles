...
pub fn setup_vim_nvim(upgrade_mode: bool, dry_run: bool, verbose: bool) {
    ...
    if Path::new(&vimrc_path).exists() {
        if let Ok(metadata) = std::fs::symlink_metadata(&vimrc_path) {
            if !metadata.file_type().is_symlink() {
                ...
                execute_command(
                    &format!("cp \"{}\" \"{}.backup.$(date +%Y%m%d_%H%M%S)\"", vimrc_path, vimrc_path),
                    "Backup .vimrc", dry_run, verbose
                );
            }
        }
    }
    ...
    let is_dir_not_empty = std::fs::read_dir(&nvim_dir).map(|mut d| d.next().is_some()).unwrap_or(false);
        ...
        if has_lazyvim_config {
            ...
            if upgrade_mode {
                ...
                if Path::new(&nvim_git).exists() {
                    ...
                    execute_command(&format!("(cd \"{}\" && git pull --rebase --autostash)", nvim_dir), "Upgrade LazyVim starter", dry_run, verbose);
                } else {
                    log_info("Skipping LazyVim starter git update (no .git metadata).");
                }
                log_info("Syncing LazyVim plugins...");
                execute_command("nvim --headless '+Lazy! sync' '+qa'", "Sync LazyVim plugins", dry_run, verbose);
            }
        } else {
            log_warn(&format!("Existing Neovim config found at {}. Skipping LazyVim installation to avoid overwriting your config.", nvim_dir));
            log_warn(&format!("Remove or back up {} and rerun setup.sh to install LazyVim.", nvim_dir));
        }
    } else {
        log_info("Installing LazyVim...");
        execute_command(&format!("rm -rf \"{}\"", nvim_dir), "Remove empty Neovim config directory", dry_run, verbose);
        execute_command(&format!("git clone \"{}\" \"{}\"", LAZYVIM_REPO, nvim_dir), "Clone LazyVim starter", dry_run, verbose);
        execute_command(&format!("rm -rf \"{}/.git\"", nvim_dir), "Remove LazyVim starter git metadata", dry_run, verbose);
        execute_command(&format!("rm -rf \"{}\"", nvim_data_dir), "Remove existing Neovim data for clean LazyVim bootstrap", dry_run, verbose);
        execute_command("nvim --headless '+Lazy! sync' '+qa'", "Install LazyVim plugins", dry_run, verbose);
