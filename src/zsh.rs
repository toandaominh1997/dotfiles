...
const ZSH_SYNTAX_HIGHLIGHTING_REPO: &str = "https://github.com/zsh-users/zsh-syntax-highlighting.git";
const ZSH_COMPLETIONS_REPO: &str = "https://github.com/zsh-users/zsh-completions.git";
const ZSH_HISTORY_SEARCH_REPO: &str = "https://github.com/zsh-users/zsh-history-substring-search.git";
...
pub fn install_or_upgrade_repo(
    repo_url: &str, dest_path: &str, repo_name: &str, upgrade_mode: bool, dry_run: bool, verbose: bool
) -> bool {
    if Path::new(dest_path).exists() {
        log_info(&format!("{} already installed at {}.", repo_name, dest_path));
        if upgrade_mode {
            log_info(&format!("Upgrading {}...", repo_name));
            if !execute_command(
                &format!("(cd \"{}\" && git pull --rebase --autostash)", dest_path),
                &format!("Upgrade {}", repo_name), dry_run, verbose
            ) {
        ...
        if !execute_command(
            ...
            &format!("Clone {}", repo_name), dry_run, verbose
        ) {
...
pub fn setup_oh_my_zsh(upgrade_mode: bool, dry_run: bool, verbose: bool) {
    ...
    if !Path::new(&oh_my_zsh_dir).exists() {
        execute_command(
            ...
            "Install Oh My Zsh", dry_run, verbose
        );
        execute_command(
            &format!("export ZSH=\"{}\" && \"{}/tools/install.sh\" --unattended --skip-chsh || true", oh_my_zsh_dir, oh_my_zsh_dir),
            "Run Oh My Zsh installer", dry_run, verbose
        );
    } else {
        ...
        if upgrade_mode {
            ...
            execute_command(
                &format!("(cd \"{}\" && git pull --rebase --autostash)", oh_my_zsh_dir),
                "Upgrade Oh My Zsh", dry_run, verbose
            );
        }
    }
}
...
pub fn setup_p10k_config(dry_run: bool, verbose: bool) {
    ...
    if let Ok(metadata) = std::fs::symlink_metadata(&dest) {
        ...
        execute_command(
            ...
            "Backup .p10k.zsh", dry_run, verbose
        );
    }

    execute_command(&format!("ln -sf \"{}\" \"{}\"", src, dest), "Symlink .p10k.zsh", dry_run, verbose);
